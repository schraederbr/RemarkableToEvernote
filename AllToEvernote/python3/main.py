...
# TODO:
# Attach based on names alone, and not dates. For each note in xochitl attempt to find note in evernote.
# Option to create note instead of attaching
# Add complete setup instructions, auto setup maybe
...
def get_todays_note(path, date=None):
    ...
    try:
        content = f"{path}/{metadata_dict[date]}.content"
        ...
        try:
            ...
            parts_no_leading_zeros = [part.lstrip('0') or '0' for part in parts]

            # Join the parts back into a string
            date = "-".join(parts_no_leading_zeros)

            content = f"{path}/{metadata_dict[date[1:]]}.content"
        except Exception as e:
            print(e)
...
def single_day(args, date):
    ...
    try:
        if(args.ocr):
            if(args.pdf):
                print('python2 note_attach.py -d ' + note_date + ' -o ' + note_date + '/' + note_date + '.txt -f ' + note_date + '/' + note_date + '.pdf')
        ...
        print(e)
        print("Failed to attach to Evernote, continuing")

def main():
    # Set up argument parser
    parser = argparse.ArgumentParser(description="Attach Remarkable notebook to Evernote note")
    parser.add_argument(
        "-p", "--path",
